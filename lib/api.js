...
export function fetchQuizQuestions(category) {
  ...
  return apiFetch(`/api/personal/quiz/questions${qs}`);
}

export function fetchQuizAnswers() {
  return apiFetch('/api/personal/quiz/answers');
}

export function submitQuizAnswer(data) {
  return apiFetch('/api/personal/quiz/answer', { method: 'POST', body: JSON.stringify(data) });
}

export function fetchQuizProfile() {
  return apiFetch('/api/personal/quiz/profile');
}
