...
export function fetchGrowthOutreach(limit = 50) {
  return apiFetch(`/api/growth/outreach?limit=${limit}`);
}

export function fetchGrowthOutreachCapacity() {
  return apiFetch('/api/growth/outreach/capacity');
}

export function draftGrowthOutreach({ prospectId, sequence, step, channel }) {
  return apiFetch('/api/growth/outreach/draft', {
    method: 'POST',
    body: JSON.stringify({ prospect_id: prospectId, sequence, step, channel }),
  });
}

export function rewriteGrowthOutreach(id, instruction) {
  return apiFetch(`/api/growth/outreach/${id}/rewrite`, {
    method: 'POST', body: JSON.stringify({ instruction }),
  });
}

export function markGrowthOutreachSent(id) {
  return apiFetch(`/api/growth/outreach/${id}/mark-sent`, { method: 'POST' });
}

export function skipGrowthOutreach(id, reason) {
  return apiFetch(`/api/growth/outreach/${id}/skip`, {
    method: 'POST', body: JSON.stringify({ reason }),
  });
}

export function recordProspectOutcome(id, outcome, notes) {
  return apiFetch(`/api/growth/prospects/${id}/outcome`, {
    method: 'POST', body: JSON.stringify({ outcome, notes }),
  });
}

export function snoozeProspect(id, days = 30) {
  return apiFetch(`/api/growth/prospects/${id}/snooze`, {
    method: 'POST', body: JSON.stringify({ days }),
  });
}

export function fetchGrowthProspects(filters = {}) {
  const params = new URLSearchParams();
  if (filters.band) params.set('band', filters.band);
  if (filters.status) params.set('status', filters.status);
  if (filters.limit) params.set('limit', filters.limit);
  return apiFetch(`/api/growth/prospects?${params}`);
}

export function fetchGrowthProspectQueue(limit = 25) {
  return apiFetch(`/api/growth/prospects/queue?limit=${limit}`);
}

export function researchGrowthProspect(id) {
  return apiFetch(`/api/growth/prospects/${id}/research`, { method: 'POST' });
}

export function fetchGrowthScoring() {
  return apiFetch('/api/growth/scoring');
}

export function fetchGrowthImportStatus() {
  return apiFetch('/api/growth/import/status');
}

export function runGrowthImport(body = {}) {
  return apiFetch('/api/growth/import', { method: 'POST', body: JSON.stringify(body) });
}

export function fetchGrowthAnalytics(days = 30) {
  return apiFetch(`/api/growth/analytics?days=${days}`);
}

export function fetchGrowthDiagnostic(days = 90) {
  return apiFetch(`/api/growth/analytics/diagnostic?days=${days}`);
}
