...
export function fetchHealthDnaCrossref() {
  return apiFetch('/api/personal/health/dna-crossref');
}
...
export function fetchDnaAtlas({ category, search, page = 1, limit = 50 } = {}) {
  const params = new URLSearchParams({ page, limit });
  if (category) params.set('category', category);
  if (search) params.set('search', search);
  return apiFetch(`/api/dna/atlas?${params}`);
}

export function fetchDnaPersonal() {
  return apiFetch('/api/dna/personal');
}

export function importDnaData(data) {
  return apiFetch('/api/dna/personal/import', { method: 'POST', body: JSON.stringify(data) });
}

export function fetchDnaAnalysis() {
  return apiFetch('/api/dna/personal/analysis');
}

export function generateDnaReport(reportType) {
  return apiFetch('/api/dna/personal/report', { method: 'POST', body: JSON.stringify({ reportType }) });
}

export function fetchDnaReports() {
  return apiFetch('/api/dna/personal/reports');
}

export function seedDnaAtlas() {
  return apiFetch('/api/dna/atlas/seed', { method: 'POST' });
}

export function deleteDnaData() {
  return apiFetch('/api/dna/personal', { method: 'DELETE' });
}

export function fetchDnaHealthCrossref() {
  return apiFetch('/api/dna/personal/health-crossref');
}
...
// ── Dating ────────────────────────────────────────────────────
export function fetchDatingProfiles(accountId) {
  return apiFetch(`/api/dating/profiles?account_id=${accountId}`);
}

export function fetchDatingProfile(platform, accountId) {
  return apiFetch(`/api/dating/profiles/${platform}?account_id=${accountId}`);
}

export function saveDatingProfile(data) {
  return apiFetch('/api/dating/profiles', { method: 'POST', body: JSON.stringify(data) });
}

export function updateDatingProfile(platform, data) {
  return apiFetch(`/api/dating/profiles/${platform}`, { method: 'PUT', body: JSON.stringify(data) });
}

export function deleteDatingProfile(platform, accountId) {
  return apiFetch(`/api/dating/profiles/${platform}?account_id=${accountId}`, { method: 'DELETE' });
}

export function optimizeDatingProfile(platform, accountId) {
  return apiFetch(`/api/dating/profiles/${platform}/optimize`, { method: 'POST', body: JSON.stringify({ account_id: accountId }) });
}

export function fetchDatingMatches(accountId, filters = {}) {
  const params = new URLSearchParams({ account_id: accountId });
  if (filters.platform) params.set('platform', filters.platform);
  if (filters.outcome) params.set('outcome', filters.outcome);
  if (filters.start_date) params.set('start_date', filters.start_date);
  if (filters.end_date) params.set('end_date', filters.end_date);
  return apiFetch(`/api/dating/matches?${params}`);
}

export function addDatingMatch(data) {
  return apiFetch('/api/dating/matches', { method: 'POST', body: JSON.stringify(data) });
}

export function updateDatingMatch(id, data) {
  return apiFetch(`/api/dating/matches/${id}`, { method: 'PUT', body: JSON.stringify(data) });
}

export function deleteDatingMatch(id) {
  return apiFetch(`/api/dating/matches/${id}`, { method: 'DELETE' });
}

export function fetchDatingAnalytics(accountId) {
  return apiFetch(`/api/dating/analytics?account_id=${accountId}`);
}

export function fetchDatingFunnel(accountId) {
  return apiFetch(`/api/dating/analytics/funnel?account_id=${accountId}`);
}

export function datingConversationHelper(conversation, context) {
  return apiFetch('/api/dating/conversation-helper', { method: 'POST', body: JSON.stringify({ conversation, context }) });
}

export function datingRedFlagCheck(text) {
  return apiFetch('/api/dating/red-flag-check', { method: 'POST', body: JSON.stringify({ text }) });
}

export function fetchDatingStats(accountId) {
  return apiFetch(`/api/dating/stats?account_id=${accountId}`);
}
...
export function fetchSportsTeams() {
  return apiFetch('/api/sports/teams');
}

export function addSportsTeam(data) {
  return apiFetch('/api/sports/teams', { method: 'POST', body: JSON.stringify(data) });
}

export function removeSportsTeam(id) {
  return apiFetch(`/api/sports/teams/${id}`, { method: 'DELETE' });
}

export function fetchSportsSchedule() {
  return apiFetch('/api/sports/schedule');
}

export function fetchSportsStandings() {
  return apiFetch('/api/sports/standings');
}

export function fetchSportsGames({ league, team } = {}) {
  const params = new URLSearchParams();
  if (league) params.set('league', league);
  if (team) params.set('team', team);
  return apiFetch(`/api/sports/games?${params}`);
}

export function rateSportsGame(data) {
  return apiFetch('/api/sports/games', { method: 'POST', body: JSON.stringify(data) });
}

export function updateSportsGame(id, data) {
  return apiFetch(`/api/sports/games/${id}`, { method: 'PUT', body: JSON.stringify(data) });
}

export function fetchSportsPlayers({ league, team, position } = {}) {
  const params = new URLSearchParams();
  if (league) params.set('league', league);
  if (team) params.set('team', team);
  if (position) params.set('position', position);
  return apiFetch(`/api/sports/players?${params}`);
}

export function fetchSportsPlayerDetail(name) {
  return apiFetch(`/api/sports/players/${encodeURIComponent(name)}`);
}

export function fetchSportsSeasonStats() {
  return apiFetch('/api/sports/season-stats');
}

export function fetchSportsToday() {
  return apiFetch('/api/sports/today');
}
...
export function fetchCompanionSettings() {
  return apiFetch('/api/companion/settings');
}

export function updateCompanionSettings(settings) {
  return apiFetch('/api/companion/settings', { method: 'PUT', body: JSON.stringify(settings) });
}

export function fetchCompanionConversations(limit) {
  const qs = limit ? `?limit=${limit}` : '';
  return apiFetch(`/api/companion/conversations${qs}`);
}

export function fetchCompanionConversation(id) {
  return apiFetch(`/api/companion/conversations/${id}`);
}

export function createCompanionConversation() {
  return apiFetch('/api/companion/conversations', { method: 'POST' });
}

export function sendCompanionMessage(conversationId, message) {
  return apiFetch(`/api/companion/conversations/${conversationId}/message`, { method: 'POST', body: JSON.stringify({ message }) });
}

export function deleteCompanionConversations() {
  return apiFetch('/api/companion/conversations', { method: 'DELETE' });
}

export function fetchCompanionCheckins(limit) {
  const qs = limit ? `?limit=${limit}` : '';
  return apiFetch(`/api/companion/checkins${qs}`);
}

export function acknowledgeCheckin(id) {
  return apiFetch(`/api/companion/checkins/${id}/acknowledge`, { method: 'POST' });
}
